...
bool Chat::is_member_banned(const std::string &mask, const std::string &prefix) { return is_equal_to_regex(mask, prefix); }

void Chat::set_limit(int limit) { client_limit = limit; }

void Chat::set_flag(unsigned char flag) { flags |= flag; }

const std::string& Chat::get_name() const { return name; }

const unsigned char& Chat::get_flags() const { return flags; }

void Chat::remove_flag(unsigned char flag) { flag &= ~flag; }
