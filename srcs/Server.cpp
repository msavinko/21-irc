...
void Server::bind_socket()
{
	...
	if(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1 || bind(socket_fd, reinterpret_cast<struct sockaddr *>(&addr), addr_len) == -1)
...
int Server::handle_mes(Client& member)
{
	while (member.get_messages().size() > 0 && member.get_messages().front()[member.get_messages().front().size() - 1] == '\n')
	{
		...
		log_message(msg);
		if(!(member.get_flags() & REGISTERED) && msg.get_command() != "QUIT" && msg.get_command() != "PASS" && msg.get_command() != "USER" && msg.get_command() != "NICK")
